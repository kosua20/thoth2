...
static size_t
char_video(hoedown_buffer *ob, hoedown_document *doc, uint8_t *data, size_t offset, size_t size) {
	size_t ret;
...
char_link(hoedown_buffer *ob, hoedown_document *doc, uint8_t *data, size_t offset, size_t size)
{
	...
	int is_vid = (offset && data[-1] == '?' && !is_escaped(data - offset, offset - 1));
	...
	if ((is_footnote && !doc->md.footnote_ref) || (is_img && !doc->md.image) || (is_vid && !doc->md.video)
		|| (!is_img && !is_footnote && !is_vid && !doc->md.link))
		goto cleanup;
...
hoedown_document_new(
	...
	size_t max_nesting, const uint8_t * media_opts, size_t media_size, int images_link)
{
	...
	if (doc->md.image || doc->md.link || doc->md.footnotes || doc->md.footnote_ref || doc->md.video) {
...
hoedown_document_free(hoedown_document *doc)
{
	...
	hoedown_buffer_free(doc->media_opts);
	free(doc);
}
