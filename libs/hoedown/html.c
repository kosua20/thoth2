...
rndr_image(hoedown_buffer *ob, const hoedown_buffer *link, const hoedown_buffer *title, const hoedown_buffer *alt, const hoedown_renderer_data *data, const hoedown_buffer *media_opts, int images_link)
{
	...
	// Use title as width for the image.
	if(state->title_as_size && title && title->size){
		HOEDOWN_BUFPUTSL(ob, "\" width=\"");
		escape_html(ob, title->data, title->size);
	} else if(media_opts && media_opts->size) {
		HOEDOWN_BUFPUTSL(ob, "\" width=\"");
		escape_html(ob, media_opts->data, media_opts->size);
	}
...
static int
rndr_video(hoedown_buffer *ob, const hoedown_buffer *link, const hoedown_buffer *title, const hoedown_buffer *alt, const hoedown_renderer_data *data, const hoedown_buffer *media_opts)
{
	hoedown_html_renderer_state *state = data->opaque;
	if (!link || !link->size) return 0;
	// playsinline needed for iOS.
	HOEDOWN_BUFPUTSL(ob, "<video autoplay loop playsinline controls");
	...
	if(state->title_as_size && title && title->size){
		...
		HOEDOWN_BUFPUTSL(ob, " width=\"");
		hoedown_buffer_put(ob, media_opts->data, media_opts->size);
		HOEDOWN_BUFPUTSL(ob, "\"");
	}
	
	HOEDOWN_BUFPUTSL(ob, ">\n");
	HOEDOWN_BUFPUTSL(ob, "<source type=\"video/mp4\" src=\"");
	escape_href(ob, link->data, link->size);
	HOEDOWN_BUFPUTSL(ob, "\" alt=\"");
...
hoedown_renderer *
hoedown_html_renderer_new(hoedown_html_flags render_flags, int nesting_level, int title_as_size)
{
	static const hoedown_renderer cb_default = {
		NULL,
	...
	state->toc_data.nesting_level = nesting_level;
	state->title_as_size = title_as_size;
